                    ...
                    "et"."price",
                        CASE
                            ...
                            CASE
                                ...
                                ELSE NULL::numeric
                            END)
                            WHEN ("et"."side" ~~* 'withdraw%'::"text") THEN ("et"."amount" * COALESCE(( SELECT "ap2"."current_price"
                               FROM "public"."asset_prices" "ap2"
                              WHERE ("upper"("ap2"."asset") = "upper"(
                                    CASE
WHEN ("et"."symbol" ~~ '%/%'::"text") THEN "split_part"("et"."symbol", '/'::"text", 1)
ELSE "et"."symbol"
                                    END))), (0)::numeric))
...
CREATE OR REPLACE VIEW "public"."v_holdings" AS
 WITH "latest_rates" AS (
         ...
         SELECT "v_all_transactions_classified"."user_id",
          ...
          GROUP BY "v_all_transactions_classified"."user_id", "v_all_transactions_classified"."entity_id", "v_all_transactions_classified"."asset"
        )
 SELECT "cq"."user_id",
    "cq"."entity_id",
    "cq"."asset",
    ...
    (("cq"."current_amount" * COALESCE("ap"."current_price", (0)::numeric)) * COALESCE(( SELECT "latest_rates"."rate"
           FROM "latest_rates"
          WHERE ("latest_rates"."target_currency" = 'JPY'::"text")), (152)::numeric)) AS "current_value_jpy",
    (("cq"."current_amount" * COALESCE("ap"."current_price", (0)::numeric)) * COALESCE(( SELECT "latest_rates"."rate"
           FROM "latest_rates"
          WHERE ("latest_rates"."target_currency" = 'EUR'::"text")), 0.94)) AS "current_value_eur",
    "now"() AS "last_updated"
...
CREATE OR REPLACE VIEW "public"."v_balance_sheet" AS
 SELECT "user_id",
    ...
    'Inventory (Trading Crypto)'::"text" AS "account",
...
CREATE OR REPLACE VIEW "public"."v_cash_flow_statement" AS
 SELECT "user_id",
    ...
    "date",
        CASE
            WHEN ("usage" = 'sales_revenue'::"text") THEN 'Inflow from Sales (IAS 2 & IFRS 15)'::"text"
            WHEN ("usage" = 'cost_of_sales'::"text") THEN 'Outflow for Inventory (IAS 2)'::"text"
            WHEN ("usage" = 'gas_fee'::"text") THEN 'Outflow for Gas Fees'::"text"
            ELSE "usage"
        END AS "item",
    "value_usd" AS "amount",
...
CREATE OR REPLACE VIEW "public"."v_profit_loss_statement" AS
 SELECT "user_id",
    ...
    "date",
        CASE
            WHEN ("usage" = 'sales_revenue'::"text") THEN 'Sales Revenue (IAS 2)'::"text"
            WHEN ("usage" = 'other_revenue'::"text") THEN 'Consideration Revenue (IFRS 15)'::"text"
            WHEN ("usage" = 'cost_of_sales'::"text") THEN 'Cost of Goods Sold (IAS 2)'::"text"
            WHEN ("usage" = 'staking'::"text") THEN 'Staking & Mining Rewards'::"text"
            WHEN ("usage" = 'mining'::"text") THEN 'Staking & Mining Rewards'::"text"
            WHEN ("usage" = 'gas_fee'::"text") THEN 'Gas & Network Fees'::"text"
            WHEN ("usage" = 'lost'::"text") THEN 'Loss of Crypto (Unrecoverable)'::"text"
            ELSE "usage"
        END AS "account",
    "value_usd" AS "amount_usd",
    "value_jpy" AS "amount_jpy",
   ...
   FROM "public"."all_transactions"
  WHERE (("usage" IS NOT NULL) AND ("type" <> 'transfer'::"text"));

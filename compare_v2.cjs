...
async function run() {
    const sbData = readJsonFile('sb_txs_final.json');
    const sbTxs = sbData.transactions || [];
    const ethTxs = parseCsv('etherscan_data.csv');

    console.log(`Statistics: ETH Records=${ethTxs.length}, SB Records=${sbTxs.length}`);
    ...
    ethTxs.forEach(tx => {
        ...
        if (!sbKeys.has(key)) {
            missingCount++;
            if (missingCount <= 50) {
                console.log(`ETH-ONLY: ${tx.hash.slice(0, 10)}... | ${asset} | ${tx.direction} | ${tx.amount}`);
            }
        }
    });
    ...
    sbTxs.forEach(tx => {
        ...
        if (!ethKeys.has(key)) {
            extraCount++;
            if (extraCount <= 50) {
                console.log(`SB-ONLY: ${tx.tx_hash.slice(0, 10)}... | ${tx.asset} | ${dir} | ${tx.amount}`);
            }
        }
    });

    const ethZeroInSb = sbTxs.filter(tx => tx.asset === 'ETH' && tx.amount === 0);
    console.log(`\n--- ETH=0 in Supabase (Count: ${ethZeroInSb.length}) ---`);
    ethZeroInSb.slice(0, 10).forEach(tx => console.log(`${tx.tx_hash.slice(0, 10)}... | ${tx.timestamp}`));

...
-- File: 20240523110000_recreate_all_transactions_with_usd_conversion.sql
...
-- File: 20240523120000_consolidated_views_rebuild.sql
...
-- File: 20240524200000_create_financial_statement_views.sql
...
-- File: 20251030_fix_journal_lines_fk.sql
-- 1) 隕ｪ繝・・繝悶Ν縺ｮ id 蝙九ｒ遒ｺ隱・
do $$
declare
  parent_type text;
  jl_count bigint;
begin
  ...
  if parent_type is null then
    -- 縺ｾ縺蟄伜惠縺励↑縺・ｴ蜷医・縲∬ｦｪ繧貞・縺ｫ菴懊ｋ・・igint蜑肴署・俄ｻ譌｢蟄俶婿驥昴↓蜷医ｏ縺帙ｋ
    execute $$
  ...
  if found then
    ...
    -- 譌｢蟄倥′遨ｺ縺ｪ繧牙ｮ牙・縺ｫ關ｽ縺ｨ縺・
    execute 'drop table public.journal_lines';
  end if;

  -- 3) 隕ｪ縺ｮ蝙九↓蜷医ｏ縺帙※蟄舌ｒ菴懈・
  ...
  -- 4) 蠢・ｦ√↑繧・RLS/Index 繧貞・莉倅ｸ・
  -- RLS縺ｯ隕ｪ縺ｮuser蛻ｶ邏・↓蜷医ｏ縺帙※蠕梧ｮｵ縺ｮ繝昴Μ繧ｷ繝ｼ繧帝←逕ｨ・磯㍾隍・ｽ懈・髦ｲ豁｢・・
...
-- File: 20251111_expand_usage_labels_for_ctx.sql

...
alter table if exists public.transaction_usage_labels
  add constraint transaction_usage_labels_tx_or_ctx_check
...
-- 2. Expand predictions table to include ctx_id/drop tx_id logic (omitted, assuming context handles surrounding lines)
-- ...

alter table if exists public.transaction_usage_predictions
  add constraint transaction_usage_predictions_tx_or_ctx_check
  check (tx_id is not null or ctx_id is not null);

-- FIX: Multi-Currency Calculations for all_transactions and financial views
-- Adds value_jpy and value_eur calculation based on value_usd and daily_exchange_rates
--
-- STRATEGY:
-- 1. Modify all_transactions to calculate value_jpy and value_eur from value_usd
-- 2. v_profit_loss_statement and v_cash_flow_statement will automatically inherit these values
...
-- Step 2: Recreate all_transactions WITH MULTI-CURRENCY CALCULATION
CREATE VIEW public.all_transactions AS
WITH 
-- Get latest USD to fiat exchange rates
...
-- Get latest JPY to USD rate for reverse conversion
jpy_to_usd AS (
...
base_transactions AS (
    ...
    FROM (
        ...
        SELECT
            ...
            END AS asset,
            et.price,
            -- Calculate value_usd from JPY trades if value_usd is null
            COALESCE(
                et.value_usd,
                CASE 
                    WHEN et.symbol LIKE '%/JPY' THEN 
                        (et.amount * et.price) * (SELECT rate FROM jpy_to_usd)
...
-- Step 3: Recreate internal_transfer_pairs
...
-- Step 4: Recreate v_all_transactions_classified
...
SELECT 
    ...
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'JPY'), 152)) AS current_value_jpy,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'EUR'), 0.94)) AS current_value_eur,
    now() AS last_updated
...
-- Step 6: Recreate v_profit_loss_statement with multi-currency
...
-- Step 7: Recreate v_balance_sheet with multi-currency
...
-- Step 8: Recreate v_cash_flow_statement with multi-currency

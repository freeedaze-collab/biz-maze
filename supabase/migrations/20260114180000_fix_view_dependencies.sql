-- FIX: v_holdings and Financial Statement Views - Asset Format Normalization
-- 
-- ROOT CAUSE: exchange_trades stores symbol as "ETH/JPY", "BTC/JPY" (pair format)
--             but asset_prices stores asset as "ETH", "BTC" (single asset format)
--             This causes v_holdings JOIN to fail (0 matches)
--
-- SOLUTION:
-- 1. Modify all_transactions to normalize asset using split_part(symbol, '/', 1)
-- 2. v_holdings will then correctly JOIN with asset_prices
--
-- DEPENDENCY CHAIN:
--   wallet_transactions, exchange_trades
--      ↓
--   all_transactions (MODIFY HERE)
--      ↓
--   internal_transfer_pairs, v_all_transactions_classified
--      ↓
--   v_holdings (JOIN with asset_prices)
--      ↓
--   v_balance_sheet, v_profit_loss_statement, v_cash_flow_statement
...
-- Step 2: Recreate all_transactions WITH ASSET NORMALIZATION
-- CRITICAL: Preserves ALL existing columns and logic, only changes asset extraction for exchange
...
SELECT
    ...
    t.description,
    t.amount,
    t.asset,  -- Now normalized: "ETH" instead of "ETH/JPY"
    t.price,
    t.value_usd,
    t.value_jpy,
    t.value_eur,
...
FROM (
    ...
    SELECT
        ...
        wt.amount,
        wt.asset AS asset,  -- wallet assets already normalized (e.g., "ETH")
    ...
    -- Exchange Trades (FIX: normalize asset from "ETH/JPY" to "ETH")
    SELECT
        ...
        CASE 
            ...
            ELSE et.amount 
        END AS amount,
        -- FIX: Extract base asset from symbol pair
        -- Before: et.symbol = "ETH/JPY" → asset = "ETH/JPY" (wrong)
        -- After:  et.symbol = "ETH/JPY" → asset = "ETH" (correct)
        CASE 
            WHEN et.symbol LIKE '%/%' THEN split_part(et.symbol, '/', 1)
            ELSE et.symbol
        END AS asset,
        et.price,
        -- FIX: Calculate value_usd from JPY trades if value_usd is null
        COALESCE(
            et.value_usd,
            CASE 
                WHEN et.symbol LIKE '%/JPY' THEN 
                    (et.amount * et.price) / NULLIF(
        ...
        -- NEW: Store quote asset for reference
        CASE 
            WHEN et.symbol LIKE '%/%' THEN split_part(et.symbol, '/', 2)
            ELSE NULL
        END AS quote_asset,
...
-- Step 3: Recreate internal_transfer_pairs (UNCHANGED)
...
-- Step 4: Recreate v_all_transactions_classified (UNCHANGED)
...
-- Step 5: Recreate v_holdings (FIXED JOIN with asset_prices)
...
-- FIX: Now works because cq.asset = "ETH" (normalized) matches ap.asset = "ETH"
LEFT JOIN public.asset_prices ap ON TRIM(UPPER(cq.asset)) = TRIM(UPPER(ap.asset))
WHERE cq.current_amount > 1e-9;

-- Step 6: Recreate Financial Statement Views (UNCHANGED logic, just rebuilding)
...
GRANT SELECT ON public.all_transactions TO service_role;
GRANT SELECT ON public.internal_transfer_pairs TO authenticated;
GRANT SELECT ON public.internal_transfer_pairs TO service_role;
GRANT SELECT ON public.v_all_transactions_classified TO authenticated;
GRANT SELECT ON public.v_all_transactions_classified TO service_role;
GRANT SELECT ON public.v_holdings TO authenticated;
GRANT SELECT ON public.v_holdings TO service_role;
GRANT SELECT ON public.v_profit_loss_statement TO authenticated;
GRANT SELECT ON public.v_profit_loss_statement TO service_role;
GRANT SELECT ON public.v_balance_sheet TO authenticated;
GRANT SELECT ON public.v_balance_sheet TO service_role;
GRANT SELECT ON public.v_cash_flow_statement TO authenticated;
GRANT SELECT ON public.v_cash_flow_statement TO service_role;

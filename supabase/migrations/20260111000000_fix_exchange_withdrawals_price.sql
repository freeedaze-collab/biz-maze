-- Fix Exchange Withdrawals Zero Value (ROBUST VERSION)
-- Corrects logic where withdrawals had 0 USD value due to missing price/fee data.
-- Uses 3-step fallback: 
-- 1. Daily Rate (Asset->USD) 
-- 2. Inverse Daily Rate (1 / USD->Asset) e.g. for JPY
-- 3. Current Price (Asset Prices) as last resort.
...
-- =================================================================
-- VIEW: all_transactions (ROBUST FIX)
-- =================================================================
...
exchange_base AS (
  SELECT
    ...
    et.amount AS amount, -- Always use base amount
    ...
    ec.connection_name,
    -- FIX: Robust Value Calculation with 3-step fallback
    COALESCE(
        et.value_usd, 
        (
            CASE 
                -- If Price is invalid or it's a transfer/withdrawal, rely on Amount * Rate
                WHEN et.side ILIKE 'withdraw%' OR et.side ILIKE 'deposit%' OR et.price IS NULL OR et.price = 0 THEN et.amount
                -- For trades, unify to Amount * Rate for consistency, or use Amount/Price logic if rate missing.
                -- Here we prioritize Amount * DerivedRate.
                ELSE et.amount 
            END
        ) * COALESCE(
             der.rate,                          -- 1. Direct Rate (Asset -> USD)
             (1.0 / NULLIF(der_inv.rate, 0)),   -- 2. Inverse Rate (1 / (USD -> Asset)) e.g. USD->JPY=150 => JPY->USD=0.0066
             ap.current_price                   -- 3. Current Price (Fallback)
           ),
        0
    ) AS value_usd, 
  ...
  -- Join 1: Direct Rate (Asset -> USD)
  LEFT JOIN public.daily_exchange_rates der
    ON et.ts::date = der.date
    AND split_part(et.symbol, '/', 1) = der.source_currency
    AND der.target_currency = 'USD'
  -- Join 2: Inverse Rate (USD -> Asset) e.g. for JPY withdrawals
  ...
  -- Join 3: Current Prices (Fallback)
...
-- 2. v_all_transactions_classified

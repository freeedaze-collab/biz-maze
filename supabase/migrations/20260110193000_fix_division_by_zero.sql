-- Fix Division by Zero Error in all_transactions view
-- Drops and recreates all views with safer calculation logic
...
-- =================================================================
-- VIEW: all_transactions (FIXED DIVISION BY ZERO)
-- =================================================================
...
exchange_base AS (
  SELECT
    ...
    ec.connection_name,
    -- Apply safe dynamic calculation fallback
    COALESCE(et.value_usd, 
             (CASE 
                WHEN et.side = 'buy' THEN et.amount 
                -- CRITICAL FIX: NULLIF(et.price, 0) prevents division by zero
                ELSE et.amount / NULLIF(et.price, 0) 
              END) * der.rate,
             0) AS value_usd, 
...
-- DEPENDENT VIEWS RESTORE (Identical to previous migration) ---

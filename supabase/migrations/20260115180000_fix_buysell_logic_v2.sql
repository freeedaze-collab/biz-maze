-- URGENT FIX: Restore correct buy/sell logic
-- 
-- PROBLEM DESCRIPTION:
-- For exchange_trades table:
-- - BUY BTC/JPY: amount = BTC received, fee_currency = JPY spent (numeric stored as text)
-- - SELL BTC/JPY: amount = JPY received, fee_currency = BTC sold (numeric stored as text)
--
-- The view should always show the CRYPTO amount, not the fiat amount.
-- For BUY: Use `amount` column (crypto received)
-- For SELL: Use `fee_currency` column (crypto sold, stored as text, needs casting)
--
...
base_transactions AS (
    ...
    FROM (
        ...
        SELECT
            ...
            -- For JPY pairs, always calculate from the correct amount, don't use stored value_usd
            CASE

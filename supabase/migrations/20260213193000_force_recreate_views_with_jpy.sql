-- Migration: Force View Restoration (Fix missing current_value_jpy)
-- This script ensures all financial views are recreated with the correct columns by using CASCADE.

-- 1. DROP ALL VIEWS WITH CASCADE to break dependencies
...
-- 2. Define Normalization Function (Ensuring it exists for views)
...
-- 3. RECREATE all_transactions (Base Layer)
...
onchain_raw AS (
  SELECT
    ...
    COALESCE(t.price_usd::numeric * COALESCE(t.amount::numeric, (t.value_wei::numeric / 1e18), 0::numeric), t.value_in_usd::numeric, 0::numeric) AS value_usd,
...
-- 4. v_all_transactions_classified

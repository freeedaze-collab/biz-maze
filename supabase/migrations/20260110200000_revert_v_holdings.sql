-- Revert v_holdings to the original logic (Strict JOIN on asset_prices)
-- This restores the definition from 20260110120000_definitive_multi_currency_fix.sql
...
SELECT
    ...
    cq.asset,
    cq.current_amount,
    ap.current_price AS current_price, -- Original: No COALESCE
    (cq.current_amount * ap.current_price) AS current_value_usd, -- Original: No COALESCE & Simple Mult
...
JOIN
    public.asset_prices ap ON cq.asset = ap.asset -- Original: JOIN (Inner)
LEFT JOIN

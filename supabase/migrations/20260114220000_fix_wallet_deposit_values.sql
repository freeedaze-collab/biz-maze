-- Migration: Fix wallet DEPOSIT value calculation and SOL in v_holdings
-- 
-- PROBLEMS:
-- 1. Wallet DEPOSIT transactions have NULL value_usd/jpy/eur
-- 2. SOL is not appearing in v_holdings (missing from asset_prices)
--
-- SOLUTIONS:
-- 1. Modify all_transactions to calculate value_usd from asset_prices for wallet transactions
-- 2. Ensure SOL is in asset_prices

-- Step 1: Insert SOL into asset_prices if not exists
INSERT INTO public.asset_prices (asset, current_price, last_updated)
VALUES ('SOL', 155.0, NOW())
ON CONFLICT (asset) DO UPDATE SET current_price = EXCLUDED.current_price, last_updated = NOW();

-- Step 2: Also ensure BTC and ETH are present
INSERT INTO public.asset_prices (asset, current_price, last_updated)
VALUES ('BTC', 98000.0, NOW())
ON CONFLICT (asset) DO UPDATE SET current_price = EXCLUDED.current_price, last_updated = NOW();

INSERT INTO public.asset_prices (asset, current_price, last_updated)
VALUES ('ETH', 3500.0, NOW())
ON CONFLICT (asset) DO UPDATE SET current_price = EXCLUDED.current_price, last_updated = NOW();

-- Step 3: Recreate all_transactions with wallet value calculation from asset_prices
...
base_transactions AS (
    SELECT
        ...
        t.connection_name
    FROM (
        -- Wallet Transactions (FIXED: Calculate value from asset_prices if NULL)
        SELECT
            ...
            wt.amount,
            wt.asset AS asset,
            CASE WHEN wt.amount IS NULL OR wt.amount = 0 THEN 0 ELSE COALESCE(wt.value_in_usd, wt.amount * COALESCE(ap.current_price, 0)) / wt.amount END AS price,
            -- FIX: If value_in_usd is NULL, calculate from asset_prices
            COALESCE(wt.value_in_usd, wt.amount * COALESCE(ap.current_price, 0)) AS raw_value_usd,
            wt.usage,
            wt.note,
            wt.type,
        ...
        LEFT JOIN asset_prices ap ON UPPER(wt.asset) = UPPER(ap.asset)

        UNION ALL

        -- Exchange Trades (UNCHANGED)
        SELECT

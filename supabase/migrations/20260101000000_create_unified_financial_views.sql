...
ALTER TABLE public.wallet_connections ENABLE ROW LEVEL SECURITY; -- ADDED FOR INTERNAL TRANSFER LOGIC
...
DROP POLICY IF EXISTS "allow_user_select_own_wallet_connections" ON public.wallet_connections; -- ADDED
...
CREATE POLICY "allow_user_select_own_wallet_connections" -- ADDED
...
-- This version implements correct double-entry accounting for exchange trades and filters internal transfers.
CREATE OR REPLACE VIEW public.all_transactions AS
WITH
-- CTE to get all addresses owned by the current user for internal transfer filtering
user_owned_addresses AS (
    SELECT wallet_address FROM public.wallet_connections
),
-- Main transaction processing CTE
unified_trades AS (
    -- Exchange Trades: Correctly interpret buy/sell logic
    SELECT
        et.user_id,
        ...
        CASE
            ...
            ELSE 'Sale for ' || et.amount::text || ' ' || split_part(et.symbol, '/', 2)
        END AS description,

        -- DEBIT: What you received.
        CASE
            WHEN et.side = 'buy' THEN abs(et.amount)                                -- You get the base asset (e.g., BTC)
            ELSE abs(et.amount)                                -- You get the quote asset (e.g., JPY)
        END AS debit_amount,
        CASE
        ...
        CASE
            WHEN et.side = 'buy' THEN et.price * et.amount                               -- You give the quote asset value (cost in JPY)
            ELSE abs(et.amount / NULLIF(et.price, 0))          -- You give the base asset (e.g., BTC quantity)
    ...
    -- On-chain Transactions: Filter out internal transfers
    SELECT
        wt.user_id,
    ...
    -- CRITICAL: Exclude transactions where both sender and receiver are user's own wallets.
    -- This assumes 'from_address' and 'to_address' columns exist in wallet_transactions.
    WHERE NOT (
        wt.from_address IN (SELECT wallet_address FROM user_owned_addresses) AND
        wt.to_address IN (SELECT wallet_address FROM user_owned_addresses)
    )
)
SELECT
    t.user_id,
    ...
    COALESCE(
        ...
        (CASE WHEN t.credit_asset = 'USD' THEN t.credit_amount ELSE
            -- If debit side is USD, use that value directly
            (CASE WHEN t.debit_asset = 'USD' THEN t.debit_amount ELSE 0 END)
         END)
    ) AS value_in_usd,
    t.usage,
    t.note,
...
SELECT
    ...
    -value_in_usd AS value_in_usd, -- We negate the value_in_usd from the original transaction
...
SELECT
    ...
    CASE
        WHEN b.total_quantity_purchased > 0 THEN b.total_cost_basis / NULLIF(b.total_quantity_purchased, 0)
        ELSE 0
    END as average_buy_price,
    -- Calculate Unrealized Capital Gain
    (b.current_amount * COALESCE(ap.current_price, 0)) - (b.current_amount *
        (CASE WHEN b.total_quantity_purchased > 0 THEN b.total_cost_basis / NULLIF(b.total_quantity_purchased, 0) ELSE 0 END)
    ) as capital_gain
FROM balances b
...
WITH pnl_base AS (
    ...
    FROM public.v_all_transactions_classified
    WHERE usage IN ('sale_ias2', 'sale_ias38', 'revenue_ifrs15') AND movement_type = 'DEBIT'
    GROUP BY user_id

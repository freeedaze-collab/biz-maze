...
SELECT 
    ...
    t.credit_amount,
    t.credit_asset,
    -- Calculate value_in_usd based on the DEBIT side (what you received)
    COALESCE(t.debit_amount * rates.rate, 0) AS value_in_usd,
    t.usage,
    t.note,
    t.source,
    t.chain
...
SELECT
    ...
    -value_in_usd AS value_in_usd, -- The value of the asset given away is negative
...
WHERE b.current_amount > 0.000001;
...
WITH pnl_base AS (
    ...
    FROM public.v_all_transactions_classified
    WHERE usage = 'sale_ias2' AND movement_type = 'DEBIT' -- Revenue is the value received
    GROUP BY user_id
    ...
    FROM public.v_all_transactions_classified
    WHERE usage = 'sale_ias2' AND movement_type = 'CREDIT'
    GROUP BY user_id

    -- Other usages mapped to P&L accounts
    UNION ALL
    SELECT user_id, 'Staking & Mining Rewards' AS account, SUM(value_in_usd) AS balance FROM public.v_all_transactions_classified WHERE usage IN ('mining_rewards', 'staking_rewards') GROUP BY user_id
    UNION ALL
    ...
    -- Placeholder for other revenue/expense types if any
    -- ... more unions here as accounting logic expands ...
)
SELECT
    user_id,
    account,
...
-- VIEW 5: v_balance_sheet (This view becomes much simpler)
CREATE OR REPLACE VIEW public.v_balance_sheet AS
SELECT 
    user_id,
    asset as account, -- Each asset is its own line item on the balance sheet
...
-- You would also union Retained Earnings from P&L, etc.
-- This part requires more advanced logic linking P&L to Balance Sheet over time.
SELECT 
    user_id, 

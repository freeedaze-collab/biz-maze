-- Migration: Ultimate Holdings Calibration, Hash Deduplication & Full Column Restoration (FIXED Column Naming)
-- This script fixes "column does not exist" errors, duplication, and incorrect signs.

-- 1. DROP VIEWS in surgical order
...
-- 3. RECREATE all_transactions (Base Layer with Hash Deduplication & Signed Amounts)
...
raw_base AS (
  -- On-chain: DISTINCT ON hash to avoid quadruple records
  (
    SELECT DISTINCT ON (t.tx_hash, t.user_id, public.normalize_crypto_asset(COALESCE(t.asset, t.asset_symbol, 'ETH')), t.amount, COALESCE(t.type, t.direction))
      ...
      -- SIGNED AMOUNT for on-chain
      CASE 
        WHEN (COALESCE(t.type, t.direction) ILIKE 'withdraw%' OR COALESCE(t.type, t.direction) IN ('send', 'out')) 
        THEN -COALESCE(t.amount, (t.value_wei / 1e18), 0)
        ELSE COALESCE(t.amount, (t.value_wei / 1e18), 0)
      END AS amount,
  ...
  SELECT
    ...
    ec.connection_name, 
    CASE WHEN m.is_base THEN COALESCE(et.value_usd, 0) ELSE (m.amount * COALESCE((SELECT rate FROM public.daily_exchange_rates WHERE source_currency = m.asset AND target_currency = 'USD' AND date <= et.ts::date ORDER BY date DESC LIMIT 1), 1.0)) END AS value_usd,
...
normalized_base AS (
  ...
  FROM raw_base r
  WHERE 
    -- STRICT DEDUPLICATION (Names match standard OR exchange records)
...
SELECT 
    ...
    quote_asset, raw_price as price, type, source, chain, wallet_address, connection_name,
    ABS(value_usd) * SIGN(amount) AS value_usd,
    (ABS(value_usd) * SIGN(amount) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'JPY' LIMIT 1), 1)) AS value_jpy,
    (ABS(value_usd) * SIGN(amount) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'EUR' LIMIT 1), 1)) AS value_eur,
    (ABS(value_usd) * SIGN(amount) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'GBP' LIMIT 1), 1)) AS value_gbp,
    (ABS(value_usd) * SIGN(amount) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'INR' LIMIT 1), 1)) AS value_inr,
    (ABS(value_usd) * SIGN(amount) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'SGD' LIMIT 1), 1)) AS value_sgd,
    usage, note, entity_id, entity_name
FROM normalized_base;

-- 4. Internal Transfer Pairs
...
-- 5. v_all_transactions_classified
...
-- 6. v_holdings (Strictly restored with ALL currency columns)
...
SELECT 
    ...
    COALESCE(ap.current_price, 0) AS current_price, (cq.current_amount * COALESCE(ap.current_price, 0)) AS current_value_usd,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'JPY' LIMIT 1), 1)) AS current_value_jpy,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'EUR' LIMIT 1), 1)) AS current_value_eur,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'GBP' LIMIT 1), 1)) AS current_value_gbp,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'INR' LIMIT 1), 1)) AS current_value_inr,
    (cq.current_amount * COALESCE(ap.current_price, 0) * COALESCE((SELECT rate FROM latest_rates WHERE target_currency = 'SGD' LIMIT 1), 1)) AS current_value_sgd,
...
SELECT p.*, (p.balance * COALESCE((SELECT rate FROM rates WHERE target_currency = 'JPY' LIMIT 1), 1)) AS balance_jpy FROM (
...
-- 8. v_cash_flow_statement
CREATE OR REPLACE VIEW public.v_cash_flow_statement WITH (security_invoker = true) AS
WITH rates AS ( SELECT target_currency, rate FROM (SELECT target_currency, rate, ROW_NUMBER() OVER(PARTITION BY target_currency ORDER BY date DESC) as rn FROM public.daily_exchange_rates WHERE source_currency = 'USD') AS r WHERE rn = 1 )
SELECT user_id, entity_id, entity_name, date, 'Operating Cash Flow'::text AS category, asset, amount, value_usd, (value_usd * COALESCE((SELECT rate FROM rates WHERE target_currency = 'JPY' LIMIT 1), 1)) AS value_jpy

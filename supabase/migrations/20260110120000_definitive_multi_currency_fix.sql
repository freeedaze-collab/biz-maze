-- supabase/migrations/20260110120000_definitive_multi_currency_fix.sql
-- PURPOSE: Ensure definitive analytical views with multi-currency support and UI-aligned column names.
-- RENAMES: value_in_usd -> value_usd for UI compatibility in all_transactions.
-- ADDS: Multi-currency columns for all major financial views.
...
onchain_base AS (
  SELECT
    ...
    COALESCE(t.usd_value_at_tx, 0) AS value_usd, -- Aligned with UI
...
exchange_base AS (
  SELECT
    ...
    ec.connection_name,
    COALESCE(et.value_usd, 0) AS value_usd, -- Aligned with UI

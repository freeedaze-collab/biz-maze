...
do $$
begin
  if not exists (select 1 from pg_policies where policyname = 'pred_select_own') then
     create policy "pred_select_own" on public.transaction_usage_predictions for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'pred_upsert_own') then
     create policy "pred_upsert_own" on public.transaction_usage_predictions for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'pred_update_own') then

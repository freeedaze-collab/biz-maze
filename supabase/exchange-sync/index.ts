...
async function binanceRequest(path: string, apiKey: string, apiSecret: string, params: any) {
  ...
  const url =
    BINANCE_ENDPOINT +
    (await signBinance(path, { ...params, timestamp }, apiSecret));
...
// Binance trades require symbol → therefore loop all symbols
...
function okxSign(method: string, path: string, params: any, apiKey: string, secret: string, passphrase: string) {
...
async function okxRequest(path: string, apiKey: string, apiSecret: string, passphrase: string, params: any) {
...
// exchange_accounts に 1 行を必ず作り、id を返す
async function ensureExchangeAccountId(supabase: any, exchange: string, external_user_id: string | null) {
  ...
  const { data, error } = await supabase
    ...
    .upsert(
      ...
      {
        onConflict: "exchange,external_user_id",
      }
    )
    .select("id")
    .maybeSingle();
...
// Transfers → exchange_transfers（←今回修正した箇所）
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (credErr) {
      return new Response(JSON.stringify({ error: "credentials_read_failed" }), {
        status: 500,
        headers: cors(origin),
      });
    }

...
function signBinance(path, params, secret) {
  ...
  return crypto.subtle.importKey("raw", keyBytes, {
    ...
    "sign"
  ]).then((key)=>crypto.subtle.sign("HMAC", key, data)).then((sig)=>Array.from(new Uint8Array(sig)).map((b)=>b.toString(16).padStart(2, "0")).join("")).then((signature)=>`${path}?${qs}&signature=${signature}`);
}
async function binanceRequest(path, apiKey, apiSecret, params) {
  const timestamp = Date.now();
  const url = BINANCE_ENDPOINT + await signBinance(path, {
    ...params,
    timestamp
  }, apiSecret);
...
// Binance: symbol 必須なので、代表的なシンボルを全巡回
...
async function fetchBinanceTrades(apiKey, apiSecret, sinceMs, untilMs) {
  const params = {
    limit: 1000
  };
...
function signBybit(path, params, key, secret) {
  ...
  return crypto.subtle.importKey("raw", keyBytes, {
    ...
    "sign"
  ]).then((k)=>crypto.subtle.sign("HMAC", k, data)).then((sig)=>Array.from(new Uint8Array(sig)).map((b)=>b.toString(16).padStart(2, "0")).join("")).then((signature)=>({
...
// DB helpers
// ------------------------------
// ✅ exchange_accounts の実スキーマに合わせて修正：
//   external_user_id だけをキーに upsert する
async function ensureExchangeAccountId(supabase, _exchange, external_user_id) {
  if (!external_user_id) return null;
  const { data, error } = await supabase.from("exchange_accounts").upsert({
    external_user_id
  }, {
    onConflict: "external_user_id"
  }).select("id").maybeSingle();
  if (error) throw new Error(`ensureExchangeAccountId: ${error.message}`);
  return data?.id ?? null;
}
...
async function saveTransfers(supabase, userId, exchange, external_user_id, kind, items) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const kinds = body.kinds?.length ? body.kinds : [
      "spot_trades",
      "deposits",
      "withdrawals"
    ];
    ...
    const { data: credRow, error: credErr } = await supabase.from("exchange_api_credentials").select("enc_blob, external_user_id").eq("user_id", userId).eq("exchange", exchange).maybeSingle();
    if (credErr) {
      return new Response(JSON.stringify({

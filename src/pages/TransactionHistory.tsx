...
export default function TransactionHistory() {
  ...
  // ===== Sync All Exchanges (exchange-sync-all -> worker) =====
  const onSync = async () => {
    ...
    try {
      ...
      const base =
        ...
        "";
      const url = `${base}/functions/v1/exchange-sync-all`;
...
error: ${json?.error ?? json?.details ?? "unknown"}`;
        setErr(msg);
        alert(msg);
      } else {
        const summaries = Object.entries(json?.results ?? {})
          .map(([ex, r]: any) => `${ex}: ${r?.inserted ?? 0}/${r?.total ?? 0}${r?.error ? ` (err: ${r.error})` : ""}`)
          .join("\n");
        alert(`Synced exchanges:\n${summaries || "no results"}`);
        await loadBalances();
        await loadTxs();
      }
    } catch (e: any) {
  ...
  return (
    <AppLayout
      ...
      subtitle="Sync everything, predict IFRS usage, and keep balances aligned."
    >
      <div className="space-y-4">
        <div className="rounded-xl border bg-gradient-to-r from-sky-50 via-white to-indigo-50 p-4">
          <p className="text-sm text-slate-700">
            If you haven’t linked any wallet yet, head to <Link to="/wallets" className="underline font-semibold">Wallets</Link> first.
            Predictions suggest IFRS-friendly usage labels that you can override before saving.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            disabled={busy}
            onClick={onSync}
            title="Sync all linked exchanges via exchange-sync-all"
          >

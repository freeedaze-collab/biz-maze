...
export default function TransactionHistory() {
  ...
  async function loadBalances() {
    // 表示専用：既存テーブルから読むだけ。無ければ空
    const { data } = await supabase.from("exchange_balances").select("asset, free, locked").order("asset");
  ...
  async function onSync() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ exchange: "binance", symbols: null, since: ymdToIso(since), until: ymdToIso(until) })
      });
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button className="px-3 py-1.5 rounded border" disabled={busy} onClick={onSync}>Sync Now</button>
        <button className="px-3 py-1.5 rounded border" disabled title="Coming soon">Predict Usage</button>
      </div>
      ...
      <section className="border rounded-xl p-4">
        ...
        {balances.length === 0 ? (
          <p className="text-sm text-muted-foreground">No exchange balance found yet. Try syncing first.</p>
        ) : (
          <ul className="text-sm">
            {balances.map(b=>(
              <li key={b.asset} className="flex justify-between border-b py-1">
                <span>{b.asset}</span><span>{b.free}</span>
              </li>
            ))}
          </ul>
        )}

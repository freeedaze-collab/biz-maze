...
// 追記: QR で署名するフロー
async function linkByWalletConnect() {
  try {
    const { EthereumProvider } = (window as any).EthereumProviderUMD || (window as any);
    const provider = await EthereumProvider.init({
      projectId: 'YOUR_WALLETCONNECT_PROJECT_ID', // 無料発行
      chains: [1,137,42161,8453], // Ethereum/Polygon/Arbitrum/Base 等
      showQrModal: true,
    ...
    if (!selected) { alert('No account from WalletConnect'); return; }

    // nonce 取得
    const FN='https://ymddtgbsybvxfitgupqy.functions.supabase.co/functions/v1/verify-wallet-signature';
    const { nonce } = await (await fetch(FN)).json();

    // 署名（WalletConnect の personal_sign は [message, address]）
    const sig: string = await provider.request({
      method:'personal_sign',
      params:[nonce, selected],
    });

    // 検証（STRICT=false なら recovered を保存）
    const r = await fetch(FN, {
      method:'POST',
      headers:{ 'content-type':'application/json' },
      body: JSON.stringify({ address: selected, signature: sig, nonce }),
    });
    const txt = await r.text();
    if (!r.ok) throw new Error(txt);
    alert('Linked by WalletConnect');
    await load();
  } catch (e:any) {
...
<button className="px-3 py-2 rounded border" onClick={linkByWalletConnect}>
  Link with Mobile Wallet (QR)
</button>

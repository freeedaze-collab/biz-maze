...
export default function VCE() {
  ...
  const [since, setSince] = useState("");  // (unused, but kept for future)
  const [until, setUntil] = useState("");  // (unused, but kept for future)
  const [symbols, setSymbols] = useState(""); // (unused, but kept for future)

  const [busy, setBusy] = useState(false);
  const [syncMessage, setSyncMessage] = useState(""); // ★ 同期の進捗メッセージ用State
  ...
  const onSaveKeys = async () => {
    ...
    // Note: This calls 'exchange-save-keys', not 'exchange-sync-all'. This is correct.
  ...
  const onSync = async () => {
    ...
    const { data: sess } = await supabase.auth.getSession();
    const headers = sess?.session?.access_token
    ...
    while (keepGoing) {
      try {
        const { error, data } = await supabase.functions.invoke("exchange-sync-all", {
          ...
          body: {
            exchange: syncExch,
            state: currentState, // Pass the current state to the function
          },
        });
        ...
        if (data.status === 'pending') {
          ...
          throw new Error("Unexpected response status from sync function.");
        }
      } catch (error: any) {
        console.error(error);
      ...
      <div className="border rounded-xl p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            placeholder="since (now unused)"
            value={since}
            onChange={(e) => setSince(e.target.value)}
          />
          <input
            className="border rounded px-2 py-1 min-w-[210px]"
            placeholder="until (now unused)"
            value={until}
            onChange={(e) => setUntil(e.target.value)}
          />
          <input
            className="border rounded px-2 py-1 min-w-[260px]"
            placeholder="symbols (now unused)"
            value={symbols}

-- SAFE FEE TRACKING MIGRATION - IMPROVED FEE_CURRENCY HANDLING
-- Handles: null fee_currency, numeric fee_currency (JPY pairs), crypto vs fiat fees
...
base_transactions AS (
    ...
    FROM (
        ...
        SELECT
            ...
            ec.connection_name,
            et.fee,
            -- IMPROVED: Infer fee_asset when fee_currency is null
            CASE
                -- If fee_currency exists and is not numeric, use it
                WHEN et.fee_currency IS NOT NULL AND et.fee_currency !~ '^[0-9]+\.?[0-9]*$' THEN et.fee_currency
                -- If withdrawal and fee_currency is null, assume fee is in the same asset
                WHEN et.side ~~* 'withdraw%' AND et.fee_currency IS NULL THEN 
                    CASE WHEN et.symbol ~~ '%/%' THEN split_part(et.symbol, '/', 1) ELSE et.symbol END
                -- If buy/sell and fee_currency is null, assume fee is in quote asset
                WHEN et.side IN ('buy', 'sell') AND et.fee_currency IS NULL AND et.symbol ~~ '%/%' THEN 
                    split_part(et.symbol, '/', 2)
                -- Otherwise use fee_currency as-is (may be null)
                ELSE et.fee_currency
            END AS fee_asset
...
fee_transactions AS (
    SELECT
        ...
        -- Auto-classify fee usage
        CASE
            WHEN bt.parent_type IN ('buy', 'deposit') THEN 'acquisition_cost'
            WHEN bt.parent_type = 'sell' THEN 'trading_fee'
            WHEN bt.parent_type IN ('send', 'withdraw', 'withdrawal') AND bt.fee_asset NOT IN ('USD', 'USDT', 'USDC', 'JPY', 'EUR') THEN 'gas_fee_noncash'
    ...
    WHERE bt.fee IS NOT NULL 
      AND bt.fee > 0 
      AND bt.fee_asset IS NOT NULL  -- Now fee_asset is inferred, so this should catch more cases
...
COMMENT ON VIEW public.all_transactions IS 'Unified transactions view with intelligent fee tracking. Infers fee_asset from transaction context when fee_currency is null. Handles crypto and fiat fees correctly.';
...
-- Expected: Should now show 3 fee rows instead of 1
